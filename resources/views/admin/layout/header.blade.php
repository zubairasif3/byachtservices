            ...
            <div class="flex flex-grow items-center justify-between px-4 py-4 shadow-2 md:px-6 2xl:px-11">
              <div class="flex items-center gap-2 sm:gap-4 lg:hidden">
                ...
                <a class="block flex-shrink-0 lg:hidden" href="index.html">
                  <img src="{{ asset('assets/images/logo.webp') }}" class="h-9" alt="Logo" />
                </a>
              </div>
                ...
                <ul class="flex items-center gap-2 2xsm:gap-4">
                  <li>
                    ...
                    <!-- Notification Menu Area -->
                        <li class="relative" x-data="{ dropdownOpen: false, notifying: true, activeTab: 'old' }" @click.outside="dropdownOpen = false">
                            <a
                            ...
                            <span :class="!notifying && 'hidden'" class="absolute -top-0.5 right-0 z-1 h-2 w-2 rounded-full bg-meta-1 {{ $notifications->isEmpty() ? 'hidden' : '' }}">
                            ...
                            <!-- Dropdown Start -->
                            <div x-show="dropdownOpen" class="absolute overflow-y-auto -right-27 mt-2.5 flex h-90 w-75 flex-col rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark sm:right-0 sm:w-80">
                            <!-- Tab Navigation -->
                            <div class="flex border-b border-stroke dark:border-strokedark">
                                {{-- <button
                                class="w-1/2 py-3 text-center text-sm font-medium text-bodydark2 dark:text-white"
                                :class="activeTab === 'new' ? 'border-b-2 border-primary' : 'text-gray-500 dark:text-gray-400'"
                                @click="activeTab = 'new'"
                                >
                                New Notifications
                                </button> --}}
                                <button
                                class="w-1/2 py-3 text-center text-sm font-medium text-bodydark2 dark:text-white"
                                :class="activeTab === 'old' ? 'border-b-2 border-primary' : 'text-gray-500 dark:text-gray-400'"
                                @click="activeTab = 'old'"
                                >
                                Notifications
                                </button>
                            ...
                            <div class="px-4.5 py-3" x-show="activeTab === 'new'">
                                {{-- <h5 class="text-sm font-medium text-bodydark2">Notification</h5> --}}
                                <ul class="flex h-auto flex-col overflow-y-auto">
                                @foreach ($notifications as $notification)
                                    <li>
                                    <a href="{{ $notification->data['url'] }}" class="flex flex-col gap-2.5 px-4.5 py-3 hover:bg-gray-2 dark:border-strokedark dark:hover:bg-meta-4">
                                        <p class="text-sm">
                                        <span class="text-black dark:text-white">{{ $notification->data['message'] }}</span>
                                        </p>
                                        <p class="text-xs">{{ $notification->data['date'] }}</p>
                                    </a>
                                    </li>
                                @endforeach
                                @if ($notifications->isEmpty())
                                    <li>
                                    <p class="text-center text-sm text-gray-500 dark:text-gray-400 py-3">No new notifications</p>
                                    </li>
                                @endif
                                </ul>
                            </div>

                            <div class="px-4.5 py-3" x-show="activeTab === 'old'">
                                {{-- <h5 class="text-sm font-medium text-bodydark2">Old Notification</h5> --}}
                                <ul class="flex h-auto flex-col overflow-y-auto">
                                @php $readNotifications = auth()->user()->readNotifications; @endphp
                                @foreach ($notifications as $notification)
                                    <li>
                                    <a href="{{ $notification->data['url'] }}" class="flex flex-col gap-2.5 px-4.5 py-3 hover:bg-gray-2 dark:border-strokedark dark:hover:bg-meta-4">
                                        <p class="text-sm">
                                ...
                                @endforeach
                                @foreach ($readNotifications as $rnotification)
                                <li>
                                    <a href="{{ $rnotification->data['url'] }}" class="flex flex-col gap-2.5 px-4.5 py-3 hover:bg-gray-2 dark:border-strokedark dark:hover:bg-meta-4">
                                        <p class="text-sm">
                                        <span class="text-black dark:text-white">{{ $rnotification->data['message'] }}</span>
                                        </p>
                                        <p class="text-xs">{{ $rnotification->data['date'] }}</p>
                                    </a>
                                </li>
                                @endforeach
                                @if ($readNotifications->isEmpty())
                                    <li>
                                        <p class="text-center text-sm text-gray-500 dark:text-gray-400 py-3">No notifications</p>
                                    </li>
                                @endif
                                </ul>
                            </div>
                ...
                <div class="relative" x-data="{ dropdownOpen: false }" @click.outside="dropdownOpen = false">
                  <a class="flex items-center gap-4" href="#" @click.prevent="dropdownOpen = ! dropdownOpen">
                    <span class="hidden text-right lg:block">
                      ...
                      <span class="block text-xs font-medium">{{ auth()->user()->getRoleNames()->first() ?? '' }}</span>
                    </span>
