    ...
    <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <nav>
            <ol class="flex items-center gap-2">
                ...
                <li class="font-medium text-primary add-heading">Add Task</li>
            </ol>
        </nav>
    </div>
    <!-- Breadcrumb End -->
                ...
                <div>
                    ...
                    >
                        ...
                        @foreach ($customer_boats as $boat)
                            <option value="{{ $boat->id }}" {{ old('customer_boat_id') == $boat->id ? 'selected' : '' }}>
                                {{ $boat->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    >
                        ...
                        @foreach ($customers as $owner)
                            <option value="{{ $owner->id }}" {{ old('company_owner') == $owner->id ? 'selected' : '' }}>
                                {{ $owner->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    >
                        ...
                        @foreach ($workers as $worker)
                            <option value="{{ $worker->id }}" data-rate="{{ $worker->hourly_rate }}" {{ old('done_by') == $worker->id ? 'selected' : '' }}>
                                {{ $worker->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    ...
                    <textarea
                        ...
                        class="w-full rounded border border-gray-300 px-4 py-2 text-sm text-black dark:border-strokedark dark:bg-form-input dark:text-white focus:outline-none focus:ring-2 focus:ring-primary"
                    >{{ old('description_action') }}</textarea>
                </div>
                <div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        // Get the select dropdown and hourly rate input fields
        ...
        // Function to calculate and set the invoice amount
        function calculateInvoiceAmount() {
            const hourlyRate = parseFloat(hourlyRateInput.value) || 0;
            const hoursWorked = parseFloat(hoursWorkedInput.value) || 0;
            const invoiceAmount = hourlyRate * hoursWorked;

            // Set the invoice amount value
            invoiceAmountInput.value = invoiceAmount.toFixed(2); // Limit to two decimal places
        }

        // Event listener for when the worker is selected
        doneBySelect.addEventListener('change', function() {
            ...
            if (hourlyRate) {
                ...
                calculateInvoiceAmount(); // Recalculate invoice amount when worker is selected
            } else {
                hourlyRateInput.value = ''; // Clear hourly rate if no worker is selected
                invoiceAmountInput.value = ''; // Clear invoice amount
            }
        });

        // Event listener for when hours worked changes
        hoursWorkedInput.addEventListener('input', calculateInvoiceAmount);

        // Initial calculation (if worker is selected and hours are entered)
