    ...
    <div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
        ...
        <h2 class="flex gap-2 items-center text-title-md2 font-bold text-black dark:text-white add-heading">
            ...
            @if (auth()->user()->hasRole('Admin'))
                <a href="{{ route('customer_boats.create') }}" class="bg-primary flex gap-2 hover:bg-opacity-80 items-center p-1 rounded text-white px-2.5" style="font-size: 14px;line-height: 1.5;">
        ...
        <nav>
            ...
            <li class="font-medium text-primary add-heading">Boats</li>
            </ol>
        </nav>
        </div>
        <!-- Breadcrumb End -->
                            ...
                            <tr class="border-b border-gray-200 hover:bg-gray-100">
                                ...
                                <td>
                                    <div class="flex items-center gap-2">
                                        @if (auth()->user()->hasRole('Admin'))
                                            <a href="{{ route('customer_boats.edit', $boat) }}" class="px-2 py-1 text-sm text-white bg-primary rounded">
                                                Edit
                                            </a>
                                            <form action="{{ route('customer_boats.destroy', $boat) }}" method="POST" class="inline-block" onsubmit="return confirm('Are you sure?');">
                                                @csrf
                                                @method('DELETE')
                                        ...
                                        @elseif(auth()->user()->hasRole('Manager'))
                                            @can('customers_boat.update')
                                                <a href="{{ route('customer_boats.edit', $boat) }}" class="px-2 py-1 text-sm text-white bg-primary rounded">
                                                    Edit
                                                </a>
                                            @endcan
                                            @can('customers_boat.delete')
                                                <form action="{{ route('customer_boats.destroy', $boat) }}" method="POST" class="inline-block" onsubmit="return confirm('Are you sure?');">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="bg-danger px-2 py-1 rounded text-sm text-white">
