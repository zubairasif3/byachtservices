...
<div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
    ...
    <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <ol class="flex items-center gap-2">
            ...
            <li class="font-medium text-primary add-heading">Edit User</li>
        </ol>
        </nav>
    </div>
    <!-- Breadcrumb End -->
                        ...
                        <tr>
                            ...
                            <td class="border border-stroke px-4 py-2 text-center dark:border-strokedark">
                                <input type="checkbox" name="permissions[{{ $entity }}.read]"
                                       value="1"
                                       {{ $userPermissions->contains('name', "{$entity}.read") ? 'checked' : '' }}>
                            </td>
                            <td class="border border-stroke px-4 py-2 text-center dark:border-strokedark">
                                <input type="checkbox" name="permissions[{{ $entity }}.create]"
                                       value="1"
                                       {{ $userPermissions->contains('name', "{$entity}.create") ? 'checked' : '' }}>
                            </td>
                            <td class="border border-stroke px-4 py-2 text-center dark:border-strokedark">
                                <input type="checkbox" name="permissions[{{ $entity }}.update]"
                                       value="1"
                                       {{ $userPermissions->contains('name', "{$entity}.update") ? 'checked' : '' }}>
                            </td>
                            <td class="border border-stroke px-4 py-2 text-center dark:border-strokedark">
                                <input type="checkbox" name="permissions[{{ $entity }}.delete]"
                                       value="1"
                                       {{ $userPermissions->contains('name', "{$entity}.delete") ? 'checked' : '' }}>
                            </td>
                            <td class="border border-stroke px-4 py-2 text-center dark:border-strokedark">
                                <input type="checkbox"
                                       class="full-access-toggle"
                                       data-entity="{{ $entity }}"
                                       {{ $userPermissions->whereIn('name', ["{$entity}.create", "{$entity}.read", "{$entity}.update", "{$entity}.delete"])->count() === 4 ? 'checked' : '' }}>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
